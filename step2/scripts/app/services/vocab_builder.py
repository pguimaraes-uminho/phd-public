...
def build_vocab(
    ...
    structural_scaffold: dict[str, Any] | None = None,
    ...
    if not llm_available:
        ...
        if structural_scaffold:
            # Hybrid arms: HARD-FREEZE the deterministic structure. Keep ONLY the
            # LLM's names, overlaid on the scaffold's structure — the model cannot
            # change type / ontological_category / target_concept / column grouping.
            # (The prompt asks for this, but this guarantees it: type & category
            # always equal the rule-based baseline; only the naming can move.)
            raw = _enforce_scaffold_structure(raw, structural_scaffold)
        if is_compare:
            import os
...
def _enforce_scaffold_structure(llm_raw: dict[str, Any], scaffold: dict[str, Any]) -> dict[str, Any]:
    """Hard-freeze the deterministic structure for the hybrid arms.

    Returns the SCAFFOLD (rule-based {concepts, relations}) with only the NAMES
    replaced by the LLM's, matched by source element. Everything structural —
    ontological_category, maps_to.type, role, target_concept, and the (possibly
    composite) column grouping — is taken verbatim from the scaffold, so the LLM
    is physically unable to alter the deterministic structure. If the LLM did not
    provide a name for some element, the scaffold's mechanical name is kept.
    """
    def _ncol(s: str) -> str:
        return re.sub(r"\s+", "", (s or "").strip().lower())

    def _ncols(source: Any) -> list[str]:
        return [_ncol(p) for p in str(source or "").split(",") if p.strip()]

    def _nent(s: Any) -> str:
        return re.sub(r"\s+", "", (str(s) if s is not None else "").strip().lower())

    # --- Build LLM name lookups (never trusted for structure, only for names) ---
    llm_concept_name: dict[str, str] = {}      # norm(source_entity) -> canonical_name
    llm_col_name: dict[str, str] = {}          # norm("entity.col")  -> property/relation name
    for c in (llm_raw.get("concepts") or []):
        se = _nent(c.get("source_entity"))
        if se and c.get("canonical_name"):
            llm_concept_name.setdefault(se, c.get("canonical_name"))
        for a in (c.get("attributes") or []):
            mt = a.get("maps_to") or {}
            name = mt.get("canonical_name") or mt.get("relation")
            if not name:
                continue
            for col in _ncols(a.get("source")):
                llm_col_name.setdefault(col, name)

    llm_rel_name: dict[tuple, str] = {}        # (norm domain, norm range) -> canonical_name
    for r in (llm_raw.get("relations") or []):
        if r.get("canonical_name"):
            llm_rel_name.setdefault((_nent(r.get("domain")), _nent(r.get("range"))), r.get("canonical_name"))

    # --- Overlay names onto a deep copy of the scaffold structure ---
    merged = copy.deepcopy(scaffold)
    for c in (merged.get("concepts") or []):
        se = _nent(c.get("source_entity"))
        if se in llm_concept_name:
            c["canonical_name"] = llm_concept_name[se]
        for a in (c.get("attributes") or []):
            mt = a.get("maps_to") or {}
            proposed = None
            for col in _ncols(a.get("source")):
                if col in llm_col_name:
                    proposed = llm_col_name[col]
                    break
            if not proposed:
                continue
            if mt.get("type") == "relation_role":
                mt["relation"] = proposed
            else:
                mt["canonical_name"] = proposed

    for r in (merged.get("relations") or []):
        key = (_nent(r.get("domain")), _nent(r.get("range")))
        if key in llm_rel_name:
            r["canonical_name"] = llm_rel_name[key]
...
def build_vocab_prompt(
    ...
    if is_compare:
        ...
        if structural_scaffold:
            # Hybrid scenarios (4/5/6): the LLM receives the deterministic
            # rule-based structural mapping and is asked to KEEP the structure
            # and only improve the (mechanical) names. This isolates the
            # semantic contribution of the LLM given a correct structure — the
            # division of labour argued for in RQ3. The scaffold shares the exact
            # output contract, so the model can echo it back with better names.
            evidence_block += (
                "\n\nSTRUCTURAL SCAFFOLD (deterministic rule-based pass — "
                "TREAT THE STRUCTURE AS FIXED AND CORRECT):\n"
                "The JSON below is a complete structural mapping of the ERD above, produced\n"
                "by a deterministic rule-based pass. Its STRUCTURE is reliable and MUST be\n"
                "preserved exactly:\n"
                "- keep every concept, attribute and relation (do not add, drop, split or merge any);\n"
                "- keep each source column's maps_to.type (property vs relation_role) unchanged;\n"
                "- keep every ontological_category, role, target_concept and multi-column grouping unchanged.\n"
                "The scaffold's NAMES are mechanical placeholders. Your ONLY task is semantic:\n"
                "replace each concept canonical_name, each relation name and each property\n"
                "canonical_name with the best domain term following the NAMING RULES above.\n"
                "Return the SAME JSON structure (same sources, same types, same groupings) with\n"
                "only the names improved.\n"
                "SCAFFOLD_JSON:\n"
                f"{json.dumps(structural_scaffold, ensure_ascii=True, indent=2)}"
            )
